...
export class AppComponent {
  ...
  ngOnInit(): void {
    ...
    // Only grab data if user is logged in
    this.checkIfAuthorized() // this promise returns either true (authorized) or false (unauthorized)
      .then(isAuthorized => { 
        if (isAuthorized) {
          this.getMessages();
          this.populateTracks();
  ...
  checkIfAuthorized(): Promise<boolean> { 
    let prom = new Promise<boolean>((resolve, reject) => {
        this.user.subscribe(
            (usr) => {
                if (usr && usr.uid){ // (firebase.auth().currentUser)
                    resolve(true);
                } else {
                    resolve(false);
                }
  ...
  getTracks(): void {
    ...
    logout() {
      this.afAuth.auth.signOut()
        .then(result =>{
          this.messages = null;                // manually have to cancel all subscriptions on logout, ugh.
          this.ownerObservable.unsubscribe();  //  For production should probably just redirect to a login page when done logging out
          this.trackObservable.forEach(to => to.unsubscribe());
          this.tracks = [];  
          this.trackOwners = [];
        });
    }
